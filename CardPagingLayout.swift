...
//
//  Created by Linto Jacob on 5/29/20.
//  Copyright © 2020 Linto Jacob. All rights reserved.
//
...
class CardLayout: UICollectionViewLayout {
    ...
    private func getAngle(for attributes: UICollectionViewLayoutAttributes) -> CGFloat {
        ...
        let distanceFromCenter = attributes.center.x - center.x
        let totalDistanceToEdge = (visibleRect.maxX - visibleRect.minX) / 2.0
        
        // Capping the factor between -1, 1
        let distanceFactor = max(-1, min(1, distanceFromCenter / totalDistanceToEdge))
